...
- **Transfer learning** — is a machine learning technique where a model trained on one task is reused as the starting point for a different, but related task. Instead of building and training a model from scratch, which takes massive amount of data, time , and computing power, you take a model that already knows basic patterns and finetune, it for your specific goal.
...
- **Human-in-the-loop** — is an AI approach that combines human intelligence with machine learning models during training, testing, or operation. Instead of letting an AI make decisions completely on its own (fully automated), a human stays in Human-in-the-Loop and reviews the model in cases it is unsure about 
